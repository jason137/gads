...
def get_data():
    ...
    encoded_genders.columns = ['gender_' + k
        for k in lbz.classes_]

    # recombine encoded data & return
    new_data = pd.concat(objs=[encoded_genders, cts_data],
        axis=1)
...
def run_model(data):
    ...
    labels = data.ix[:, -1].map(lambda k: int(k / 5))
    ...
    # show feature importances (can be useful for
    # feature selection)
