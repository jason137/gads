...
class WordCount(MRJob):
    ...
    def mapper(self, _, line):
        ...
        # NOTE use counters to keep track of things
        # (across nodes)
        self.increment_counter('mapper', 'input lines', 1)
...
class DistrGrep(MRJob):
    ...
    def mapper(self, _, line):
        self.increment_counter('mapper', 'total lines', 1)

        if self.PATTERN in line:
            self.increment_counter('mapper', 'matching lines', 1)
            yield line, 1

    # NOTE no reducer necessary here; what difference
    # would including it make in the output?
...
class DistrDB(MRJob):
    ...
    def mapper_init(self):
        ...
        # initialize db connection for mapper in
        # mapper_init method!
        self.db = MongoClient()['test']
        self.mongo = self.db['bikes']

        # NOTE this drops test_coll at the top of
        # each job...why is this needed?
        # self.mongo.drop()
    ...
    # NOTE suppose this job had a reducer phase that required
    # db access; would it be necessary to re-initialize the
    # connection?
