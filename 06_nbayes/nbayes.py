...
def nltk_featurize(word):
    """Returns feature dict (in format nltk expects) for a given 
    word."""
    # NOTE relative quality of features can be seen from
    # likelihood ratios
    ...
    # return dict(('contains_' + k, k in word.lower())
    #     for k in string.lowercase)

    # two features!
    return {'last_two': word[-2:],
        'ae_in_name': 'ae' in word.lower()}

def nltk_model():
    """Fits the (non-parametric) naive Bayes classifier from nltk on the
    names dataset."""
    ...
    with open(MALE_FILE, 'r') as f:
        for line in f:
            ...
            all_names.append((line.rstrip(), 'male'))   
    ...
    features = [(nltk_featurize(name), gender)
...
def sklearn_model():
    """Fits the (parametric) Gaussian Naive Bayes classifier from sklearn
    on the iris dataset."""
    ...
    # show accuracy pct
    print 'accuracy = {0} %'.format(
        round(100 * gnb.score(test_features, test_labels)))
